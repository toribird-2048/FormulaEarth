...
def generate(min,max,count):
    ...
    for k in range(count):
        earth.append(np.array([rng.uniform(min,max) for k in range(length)]))
...
def mutation():
    ...
    for k in range(len(earth)):
        if rng.uniform(0,1) < mutation_rate :
            ...
            earth[k] += np.array([rng.uniform(-mutation_range,mutation_range) if k == mutation_place else 0 for k in range(length)])
